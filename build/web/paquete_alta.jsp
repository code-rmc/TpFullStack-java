...
<html lang="es"> 
<head>
    <title>Nuevo Paquete Turistico</title>
    ...
    <div class="app-wrapper">
	    ...
	    <div class="app-content pt-3 p-md-3 p-lg-4">
		    <div class="container-xl">			    
			    <h1 class="app-page-title">Registro de Nuevo Paquete Turistico</h1>
			    <hr class="mb-4">
                <div class="row g-4 settings-section">
	                <div class="col-12 col-md-4">
		                <h3 class="section-title">Datos</h3>
		                <div class="section-intro">Formulario para ingresar un nuevo paquete turistico. Cada grupo de formulario requerido tiene un estado de validación que se puede activar al intentar enviar el formulario sin completarlo. <a href="help.html">Learn more</a></div>
	                </div>
		                ...
		                <div class="app-card app-card-settings shadow-sm p-4">
						    ...
						    <div class="app-card-body">
                                                        <%
                                                            ...
                                                            List<Servicio> listServ = control.traerServicios();
                                                            
                                                        %>
                                                        <form class="settings-form row" action="SvPaqueteAlta" method="POST">
								    <div class="col-md-12 mb-3">
                                                                        <label for="setting-input-1" class="form-label">Nombre de paquete turistico<span class="ms-2" data-container="body" data-bs-toggle="popover" data-trigger="hover" data-placement="top" data-content="This is a Bootstrap popover example. You can use popover to provide extra info."><svg width="1em" height="1em" viewBox="0 0 16 16" class="bi bi-info-circle" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
<path fill-rule="evenodd" d="M8 15A7 7 0 1 0 8 1a7 7 0 0 0 0 14zm0 1A8 8 0 1 0 8 0a8 8 0 0 0 0 16z"/>
<path d="M8.93 6.588l-2.29.287-.082.38.45.083c.294.07.352.176.288.469l-.738 3.468c-.194.897.105 1.319.808 1.319.545 0 1.178-.252 1.465-.598l.088-.416c-.2.176-.492.246-.686.246-.275 0-.375-.193-.304-.533L8.93 6.588z"/>
<circle cx="8" cy="4.5" r="1"/>
</svg></span></label>
                                                                        <input type="text" class="form-control" id="setting-input-1" value="Paquete Turistico gold" name="nombre" required>
                                                                    </div>
								    <div id="listaServicios" class="col-6">
                                                                        <label class="form-label">Servicio</label>
                                                                        <%
                                                                            if(listServ.size() != 0){
                                                                                for (Servicio serv : listServ) {
                                                                                    String nomb = serv.getNombre() + " - $" + serv.getCosto_servicio();
                                                                        %>
                                                                        <div class="form-check mb-3">
                                                                            <input class="form-check-input" type="checkbox" name="servicio" value="<%=serv.getCodigo_servicio()%>" id="settings-switch-<%=serv.getCodigo_servicio()%>">
                                                                            <label class="form-check-label" for="settings-switch-<%=serv.getCodigo_servicio()%>"><%=nomb %></label>
                                                                        </div>
                                                                        <%
                                                                                }
                                                                            } else {
                                                                        %>
                                                                        <div class="form-check mb-3">
                                                                            <input class="form-check-input" type="checkbox" id="settings-switch" disabled="disabled" required="true">
                                                                            <label class="form-check-label" for="settings-switch">No hay servicios cargados</label>
                                                                        </div>
                                                                        <%
                                                                            }
                                                                        %>
                                                                    ...
                                                                    <div class="col-md-4 mb-3">
                                                                        <label for="setting-input-5" class="form-label">Costo total del Paquete</label>
                                                                        <input type="text" class="form-control" id="total" value="0" name="costo" disabled>
                                                                    </div>
                                                                
                                                                    <button type="submit" class="btn app-btn-primary">Guardar Servicio</button>
							    </form>
						    </div><!--//app-card-body-->
    ...
    <script>
        const listaServicios = document.querySelectorAll("#listaServicios .form-check");
        const inputTotal = document.getElementById("total");
        let costoTotal = 0;
        listaServicios.forEach(ele => {
            const chek = ele.children[0];
            const label = ele.children[1];
            chek.addEventListener("change", function(){
                if(this.checked) {
                    costoTotal += Number(label.textContent.slice(label.textContent.indexOf("$")+1));
                }else {
                    costoTotal -= Number(label.textContent.slice(label.textContent.indexOf("$")+1));
                    costoTotal<0 ? costoTotal=0 : costoTotal;
                    
                }
                inputTotal.value = costoTotal-costoTotal*0.1;
            })
        });
